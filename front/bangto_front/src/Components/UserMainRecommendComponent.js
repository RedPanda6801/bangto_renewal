...
const UserMainRecommendComponent = () => {
  ...
  useEffect(() => {
    axios.get(`${API.ITEM.LIST}/1`).then((res) => {
      setRecommendItems(resContent(res));
      console.log(res);
    });
  ...
  useEffect(() => {
    if (
      ...
      axios
        .get(
          `${process.env.REACT_APP_BACKEND_SERVER_PORT}/favorite/get-list/1`,
          {
  ...
  return (
    <>
      ...
      <div className="box_Wishlist">
        <div className="text_Wishlist">찜 목록</div>
        <div className="wishlist_Item">
          {!localStorage.getItem("token") ||
          localStorage.getItem("token") === undefined ? (
            <div className="wishlist_Not_Login">로그인 후 이용 가능합니다.</div>
            ...
            <div className="wishlist">
              <button
                ...
                style={{
                  cursor: currentWishlist === 0 ? "default" : "pointer",
                }}
              >
                &#10094;
              </button>
              <div className="wishlist_Row">
                <div className="wishlist_Item_Container">
                  {wishlistItems
                    .slice(
                      currentWishlist * itemsPerPage,
                      (currentWishlist + 1) * itemsPerPage
                    )
                    .map((wishlistItem, idx) => (
                      <div
                        className="wishlist_Item_Card"
                        key={`wishlistItem${idx}`}
                      >
                        <img
                          onClick={() =>
                            navigate("/user/item", {
                              state: { itemPk: wishlistItem.item.id },
                            })
                          }
                          className="wishlist_Item_Image"
                          src={`${
                            process.env.REACT_APP_IMG_PUBLIC_URI
                          }/03_upload/${wishlistItem.item.img.split("/")[0]}`}
                          alt="물품 이미지"
                        />
              ...
              {currentWishlist <
                Math.ceil(totalWishlistItems / itemsPerPage) - 1 && (
                <button
            ...
            <div className="wishlist_No_Item">찜 목록을 추가해보세요!</div>
          )}
        </div>
      </div>
      <div className="box_Itemlist">
        <div className="text_Recommend">추천상품</div>
        {recommendItems.length === 0
          ? null
          : recommendItems.map((recommendItem, idx) => {
              return (
                <div className="table_Itemlist" key={`recommendItem${idx}`}>
                  <img
                    className="itemlist_Item_Image"
                    onClick={() =>
                      navigate("/user/item", {
                        state: { itemPk: recommendItem.id },
                      })
                    }
                    src={`${process.env.REACT_APP_IMG_PUBLIC_URI}/03_upload/${
                      recommendItem.img.split("/")[0]
                    }`}
                    alt="물품 이미지"
                  />
                  <div className="itemlist_Item_Title">
                    {recommendItem.title}
                  </div>
                  <div className="itemlist Item_Price">
                    {recommendItem.price}원
                  </div>
                </div>
              );
            })}
