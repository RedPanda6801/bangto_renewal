...
const UserMainComponent = forwardRef((props, ref) => {
  ...
  const itemSearch = async () => {
    try {
      const response = await axios.get(`${API.ITEM.FILTERED_LIST}`);
  ...
  return (
    <div className="layout_Main_Contents">
      <div className="box">
        ...
        <div className="box_Search">
          <div className="box_Search_Container">
            ...
            />
            <button className="main_Search_Btn" onClick={handleSearch}>
              검색
            </button>
          </div>
        </div>
      ...
      {selectedCategory === "Main" || selectedCategory === undefined ? (
        ...
        <div className="box_No_Result">검색 결과가 없습니다.</div>
      ) : (
        <div>
          ...
          <div className="box_Itemlist">
            {items.map((item, idx) => {
              return (
                <div className="table_Itemlist" key={`searchedItem${idx}`}>
                  <img
                    className="itemlist_Item_Image"
                    onClick={() =>
                      navigate("/user/item", { state: { itemPk: item.id } })
                    }
